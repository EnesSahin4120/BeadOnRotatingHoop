...
Bead::Bead(int sectorCount, int stackCount, float radius)
{
	this->radius = radius;
	angle = 90.0f;
	acceleration = 0.0f;
...
void Bead::MovementOnHoop(float hoopRadius, float hoopVelocity, GLfloat deltaTime)
{
	acceleration = -(glm::sin(angle * toRadians) * (g / hoopRadius - hoopVelocity * hoopVelocity * glm::cos(angle * toRadians)));
	acceleration *= 150.0f;
